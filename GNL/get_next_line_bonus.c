...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/17 19:44:42 by gautier           #+#    #+#             */
/*   Updated: 2023/12/21 11:53:39 by gschwand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || read(fd, 0, 0) < 0 || BUFFER_SIZE <= 0 || fd > 1025)
		return (NULL);
	elem = ft_lstnew_n_addfront(NULL, NULL);
	elem = ft_read_alloc(fd, rest[fd], elem);
	rest[fd] = elem->content;
	elem->content = NULL;
	line = ft_write(&(elem->next), elem->next);
	if (!*line)
		return (ft_lstclear(&elem, free), free(line), free(rest[fd]), NULL);
